...
function offsetPosition(actionType: String, applyTo: any, alignment: String, nodeBefore: any, nodeNow: any) {
  ...
  switch (xAlignment) {
    ...
    case ("c"): //align center
      ...
      if (actionType === "set-x-y" && applyTo.x) { nodeNow.x -= nodeNow.width / 2 }
      break
    case ("r"): //align right
      nodeNow.x -= (nodeNow.width - nodeBefore.width)
      if (actionType === "set-x-y" && applyTo.x) { nodeNow.x -= nodeNow.width }
      break
  }
  ...
  switch (yAlignment) {
    ...
    case ("m"): //align middle
      ...
      if (actionType === "set-x-y" && applyTo.y) { nodeNow.y -= nodeNow.height / 2 }
      break
    case ("b"): //align bottom
      nodeNow.y -= (nodeNow.height - nodeBefore.height)
      if (actionType === "set-x-y" && applyTo.y) { nodeNow.y -= nodeNow.height }
      break
  }
}
